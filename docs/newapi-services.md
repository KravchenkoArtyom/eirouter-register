# Сервисы на NewAPI: eirouter, wisdomsatan и свои

NewAPI (и родственный one-api) — популярная панель для доступа к моделям по
ключу. Сайты на ней похожи как близнецы: форма регистрации (логин, пароль,
иногда код из письма), вход и личный кабинет, где выдаётся ключ `sk-…`.
Поэтому регистратор здесь универсален, а специфика живёт в двух местах:

| Что нужно | Где это описать | Код Python |
| --- | --- | --- |
| Только аккаунт (почта + пароль) | сценарий в `universal_scenarios/` — снимите поля инспектором | не нужен |
| Аккаунт **и** ключ из кабинета | регистратор в `providers/` (пример — `providers/eirouter.py`) | нужен небольшой |

Захват ключа приходится писать руками, потому что кабинет у каждой сборки
свой: `providers/eirouter.py` открывает раздел ключей, закрывает всплывающие
объявления, нажимает «скопировать» и проверяет, что в буфер попал полный
`sk-…`, а не маскированный огрызок. Почта, прокси, профили браузера, паузы и
капча при этом общие — их писать заново не нужно.

Ниже — устройство двух готовых регистраторов (перенесено из прежнего README;
поведение провайдеров не менялось).

Standalone Windows registration tool. It does not import or require zcode-farm.

Run `scripts\register_eirouter.bat`, enter the account count, then choose the mailbox
...
The launcher `scripts\register_eirouter.bat` uses interactive mode: first enter the account count, then
